  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-10">
      ...
      <section className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
        <h2 className="font-serif text-lg" style={{ color: 'var(--color-primary)' }}>Profile</h2>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-forest-500 focus:border-transparent"
            />
          </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 rounded-lg text-sm font-medium text-white disabled:opacity-60 transition-opacity hover:opacity-90"
            style={{ backgroundColor: 'var(--color-btn-bg, #27500a)' }}
          >
            {profileSaving ? 'Saving…' : 'Save profile'}
          </button>
          {profileMsg && (
            <span className={`text-sm ${profileMsg.type === 'ok' ? 'text-forest-600' : 'text-red-600'}`}>
              {profileMsg.text}
            </span>
          )}
        </div>
      ...
      <section className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
        <div>
          <h2 className="font-serif text-lg" style={{ color: 'var(--color-primary)' }}>Learning Tier</h2>
          ...
          {Object.entries(TIERS).map(([key, tier]) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl border-2 p-4 text-left transition-colors focus:outline-none focus:ring-2 focus:ring-forest-500 focus:ring-offset-2 ${
                  isChosen
        ...
        <div className="flex items-center gap-3">
          ...
          {tierMsg && (
            <span className={`text-sm ${tierMsg.type === 'ok' ? 'text-forest-600' : 'text-red-600'}`}>
              {tierMsg.text}
            </span>
          )}
        </div>
      ...
      <section className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
        <div>
          <h2 className="font-serif text-lg" style={{ color: 'var(--color-primary)' }}>Personalisation</h2>
          <p className="text-sm text-gray-500 mt-1">
        ...
        {suggestedTier && !dismissedSuggestion && suggestedTier !== currentTier && (
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gold-300 bg-gold-100 px-4 py-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="text-xs px-3 py-1.5 rounded-lg font-medium border border-gold-400 text-gold-700 hover:bg-gold-200"
              >
                Dismiss
              </button>
            </div>
            ...
            return (
              <div key={q.id} className="space-y-2">
                <div className="flex items-start justify-between gap-2">
                  ...
                  {!answered && (
                    <span className="flex-shrink-0 text-xs text-gray-400 italic mt-0.5">Not answered yet</span>
                  )}
                </div>
                  ...
                  {q.options.map((opt, optIdx) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={isSaving}
                        className={`w-full rounded-lg border px-4 py-2.5 text-left text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-forest-500 focus:ring-offset-1 disabled:opacity-60 ${
                          isSelected
                            ? 'border-forest-600 bg-sage-100 text-forest-700 font-medium'
                            : 'border-gray-200 bg-white text-gray-700 hover:border-forest-400 hover:bg-sage-50'
                        }`}
                      >
                        {opt.label}
                      </button>
      ...
      <section className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
        <div>
          <h2 className="font-serif text-lg" style={{ color: 'var(--color-primary)' }}>Subject Interests</h2>
          <p className="text-sm text-gray-500 mt-1">
        ...
        <div className="flex items-center gap-3">
          ...
          {subjectMsg && (
            <span className={`text-sm ${subjectMsg.type === 'ok' ? 'text-forest-600' : 'text-red-600'}`}>
              {subjectMsg.text}
            </span>
          )}
        </div>
