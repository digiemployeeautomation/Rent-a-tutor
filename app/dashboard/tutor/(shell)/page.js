    ...
    async function load() {
      ...
      const [
        ...
        supabase.from('lessons')
          ...
          .limit(10),
        supabase.from('bookings').select('*', { count: 'exact', head: true }).eq('tutor_id', user.id).eq('status', 'completed'),
      ...
      const rentalRevenue   = lessRows.reduce((sum, l) => sum + ((l.purchase_count ?? 0) * (l.price ?? 0)), 0)
      const sessionEarnings = bookRows.filter(b => b.status === 'completed').reduce((sum, b) => sum + (b.amount ?? 0), 0)
