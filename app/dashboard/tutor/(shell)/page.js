  ...
  useEffect(() => {
    async function load() {
      ...
      const [
        ...
        { data: recentBooks },
        // FIX: separate query for ALL lessons (no limit) so earnings are accurate
        { data: allLessons },
      ] = await Promise.all([
        ...
        supabase.from('bookings')
          ...
          .limit(10),
        // All lessons without limit — used only for earnings calculation
        supabase.from('lessons')
          .select('purchase_count, price')
          .eq('tutor_id', user.id),
      ])
      ...
      // FIX: use allLessons (complete list) not recentLess (capped at 10)
