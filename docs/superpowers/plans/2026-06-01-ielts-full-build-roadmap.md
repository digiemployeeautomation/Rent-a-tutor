...
- **Reading** ✅ — deterministic grader (Wave 1) + passage/questions UI (QuestionSetRunner via the 3 primitives) + results + academic seed set
  ...
  - **Path expansion** ✅ (2026-06-17) — four more worked modules authored against the TFNG template, taking the path to 5 modules (pos 1–5): Yes/No/Not Given, Sentence completion, Multiple choice, Matching headings. Each is a self-contained seed (`supabase/seed/seed_ielts_reading_<type>_lesson.sql`): a `skill_lessons` row + a single-type drill `practice_item` (with per-question explanations) covering all 3 primitives (single_select / text_fill). Remaining types (ynng done; matching information/features/sentence-endings, mcq_multi, summary/note/table/flow-chart/diagram completion, short answer) still to author. **Manual go-live:** after migration 008, run the four new seed files in the Supabase SQL editor (the capstone exam now unlocks only after all 5 drills are graded).
